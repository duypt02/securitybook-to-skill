...
.. note:: Legacy rule-based generator — kept for comparison
    This file implements the original rule-based approach to skill generation
    and is no longer the intended production path.

    The current approach delegates skill writing to AI harnesses (Claude Code,
    Codex, GitHub Copilot) via the prompt contract in
    ``docs/harness/codex-securitybook-to-skill-prompt.md``.  Those harnesses
    read ``full_text.txt`` and ``metadata.json`` directly and generate all
    required artifacts without calling this script.

    This file is intentionally retained as a **comparison baseline**: running
    both paths on the same source lets you measure the quality difference
    between deterministic rule-based output and harness-generated output.
